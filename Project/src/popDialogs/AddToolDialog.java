...
public class AddToolDialog extends JDialog implements ActionListener {
	...
	private EditBrushDialog dialogEdit;
	
	public AddToolDialog() {
		
		Window parent = SwingUtilities.getWindowAncestor(this);
		
		setTitle("Dodavanje softverskog alata");
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(tool == null) {
						...
						if(SoftwareToolController.getInstance().uniqueToolName(tool))
						{
							SoftwareToolController.getInstance().addTool(tool);
							dispose();
						}else {
							JOptionPane.showMessageDialog(parent, "Naziv softverskog alata nije jedinstven!");
							okButton.setToolTipText(" Naziv softverskog alata nije jedinstven!");
							okButton.setBackground(Color.RED);
							okButton.setForeground(Color.WHITE);
						}
		...
		setLayout(okButton,cancelButton, brushesF, editBrushes);
	...
	private void setLayout(JButton okButton, JButton cancelButton, JButton addBrush, JButton editBrushes) {
		
		FlowLayout buttonsLayout = new FlowLayout();
		final JPanel brushButtons = new JPanel();
		brushButtons.setLayout(buttonsLayout);
		brushButtons.add(addBrush);
	...
	private void openEditBrushesDialogAction() {
		 if (dialogEdit == null) {
	         Window win = SwingUtilities.getWindowAncestor(this);
	         if (win != null) {
	        	dialogEdit = new EditBrushDialog(brushes);
	         }
	      }
